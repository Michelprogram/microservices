...
const RIDES_BASE_URL =
  process.env.RIDES_SERVICE_URL || "http://rides-service:8080";
    ...
    .action(async () => {
      try {
        ...
        const response = await usersApi.patch(
          `/drivers/${driverAnswer.driverId}/status`,
          {
    ...
    .action(async () => {
      try {
        ...
        const response = await usersApi.get(
          `/passengers/${answers.passengerId}`
        );
    ...
    .action(async () => {
      try {
        ...
        const response = await usersApi.put(
          `/passengers/${passengerAnswer.passengerId}`,
          {
      ...
      try {
        ...
        const answers = await inquirer.prompt([
          ...
          },
          {
            ...
            choices: ["Centre-ville", "Aéroport", "Plateau", "McGill"].map(
              (zone) => ({
                name: zone,
                value: zone,
              })
            ...
            validate: (input) =>
              input.trim() ? true : "Pickup zone is required",
          },
          {
            type: "list",
            name: "to_zone",
            ...
            validate: (input) =>
              input.trim() ? true : "Destination zone is required",
          },
        ]);
        ...
        const answers = await inquirer.prompt([
          {
            type: "input",
            name: "rideId",
            message: "Enter ride ID:",
            validate: (input) => (input.trim() ? true : "Ride ID is required"),
          },
        ]);
  ...
  rideCommand
    ...
    .action(async () => {
      try {
        ...
        const response = await ridesApi.patch(
          `/rides/${answers.rideId}/status`,
          {
    ...
    .action(async () => {
      while (true) {
        ...
        if (action === "drivers") {
          ...
          const driverCommand = program.commands.find(
            (cmd) => cmd.name() === "drivers"
          );
          if (driverCommand) {
            const subCommand = driverCommand.commands.find(
              (cmd) => cmd.name() === driverAction
            );
            if (subCommand) {
        ...
        if (action === "passengers") {
          ...
          const passengerCommand = program.commands.find(
            (cmd) => cmd.name() === "passengers"
          );
          if (passengerCommand) {
            const subCommand = passengerCommand.commands.find(
              (cmd) => cmd.name() === passengerAction
            );
            if (subCommand) {
              await subCommand.parseAsync([], { from: "user" });
            }
        ...
        if (action === "rides") {
          ...
          const rideCommand = program.commands.find(
            (cmd) => cmd.name() === "rides"
          );
          if (rideCommand) {
            const subCommand = rideCommand.commands.find(
              (cmd) => cmd.name() === rideAction
            );
            if (subCommand) {
              await subCommand.parseAsync([], { from: "user" });
            }
